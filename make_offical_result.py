...
parser.add_argument('--mode', type=str, default='sorted',
                    choices=['single', 'sorted'])
# how to combine observations:
# single: chose the biggest top-1 score and its rank from single image
...
out_name = f"results/clef_plant_submission_epoch{epoch}_{mode}.csv"
    ...
    for i in range(len(value)):
        ...
        if int(obs_id) != history_obs_id and i > 0:
            ...
            for num in range(max_num):
                single = str(obs_id) + ';' + str(int(label[num])) + ";" + str(score[num]) + ";" + str(num) + ";" + "\n"
            ...
            if top1_score > history_score or i == 0:
                ...
                content = []
                for num in range(max_num):
                    single = str(obs_id) + ';' + str(int(label[num])) + ";" + str(score[num]) + ";" + str(num + 1) + ";" + "\n"
                    content.append(single)
